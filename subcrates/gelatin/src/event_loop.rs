...
use crate::{application::Application, window::{Window, WindowDescriptor}};
    ...
    UserEvent: Debug + 'static,
{
    pub fn new() -> Self {
        Self {
            inner: WinitEventLoop::<UserEvent>::with_user_event().build().unwrap(),
        }
    }
...
impl<'a> ActiveEventLoop<'a> {
    pub fn create_window(&mut self, desc: WindowDescriptor) -> Result<Rc<Window>, Box<dyn std::error::Error>> {
        Ok(Window::new(
            self.application,
            desc,
            self.inner,
